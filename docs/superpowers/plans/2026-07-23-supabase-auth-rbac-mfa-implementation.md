...
## 2026-07-23 preflight outcome（Phase-B migration gate）

**判定: NO-GO for Phase-B migration creation in this run.** 外部Supabase環境へのmigration適用も引き続きNO-GO。

- **FACT:** project-scoped `read_only=true` MCPのdedicated migration listは0 rowsだったが、repositoryには4 migration fileがある。
- **FACT:** live `public`の12 base tableはすべてRLS disabledで、proposed Auth-foundation 8 tableは報告されなかった。
- **UNVERIFIED:** grants/default privileges、functions/RPC、triggers、policies、FORCE RLS、company-ID aggregate counts。read-only catalog `execute_sql`がMCP endpointでcancelされた。
- **FACT:** checked-in chainは6 legacy base tableを作成せず、後続migrationは`applicants`を無条件にalter/referenceする。空local databaseをresetできるmigration chainとは仮定しない。
- **FACT:** 正式設計と旧計画はRLS sequencing、`legacy_company_key`、table別timestamp/FK delete action、owner invariantの実装境界について未解決である。
- **PROPOSAL:** Phase 1Aはこれらのevidenceとdecisionが明示的に解決されるまでblockedとする。このrunではmigration、SQL test、`docs/superpowers/plans/2026-07-23-auth-foundation-schema.md`を作成しない。

このoutcomeは旧Phase-1 sequencingを次の範囲でsupersedeする。

1. public schemaへ作る新tableは、作成する同一transaction内でRLSを有効化する。利用者allow policyは後続の承認済み実装まで追加せず、initial stateをdeny-by-defaultとする。migration 4までRLS有効化を遅らせない。
2. `companies.legacy_company_key`は正式設計に含まれないため、正式設計に対する別承認が得られるまでPhase 1Aから除外する。旧計画の記述だけを承認根拠にしない。
3. partial unique owner indexが保証するのは「1企業につきactive owner最大1件」である。ownerが常に1人以上存在すること、owner変更が運営専用transactionに限定されることは、transaction function、grant、policy、testが揃うまで実装済みと主張しない。
4. tableごとのtimestamp列とFK delete actionはmatrixとして決定してからSQLを作る。

後続Phaseの目的は変更しない。Phase 1Aを再開するには、migration-history mismatch、catalog evidence、base DDL/rehearsal方法、および上記設計判断を先に解決する。
...
既存 `company_id text` を一度にUUIDへ変更しない。UUID bridgeの列・対応表・backfill方式は、正式設計に対する別承認後のphaseへdeferする。承認済み方式でbridgeを導入する場合も、アプリとRLSをUUIDへ切り替えた後にだけ正式な `company_id` へcutoverし、移行途中は現行固定企業経路へ戻せる境界を維持する。

### 3.2 採用しない方式

- **業務tableのallow policy先行:** 利用者JWT、所属、AAL2、企業選択がないため正しいpolicyを評価できない。新規Auth table自体のRLS enablementはtable作成transaction内で先行し、policyなしのdeny-by-defaultとする。
...
|---|---|---|---|
| 1 | `supabase/migrations/202607230001_auth_core.sql` | companies、profiles、company_members、platform_adminsを作成し、同一transactionで各tableのRLSをenable。user allow policyは追加しない | 未使用なら新規テーブルを依存順にdrop |
| 2 | `supabase/migrations/202607230002_auth_invites_sessions_audit_exports.sql` | invitations、app_sessions、audit_logs、data_exportsを作成し、同一transactionで各tableのRLSをenable。user allow policyは追加しない | 未使用なら新規テーブルを依存順にdrop |
...
| 4 | `supabase/migrations/202607230004_auth_table_policies.sql` | 既にRLS enabledの新規認証テーブルへ承認済みpolicyと最小grantを追加 | policy/grantを戻す。RLS enabledとテーブルは維持 |
...
- 部分一意indexは企業ごとのactive ownerを**最大1件**に制限する。このindexだけではownerが常に1件存在することも、owner移譲経路も保証しない。minimum/transaction-only invariantは後続の承認済みfunction、grant、policy、testで実装する。
...
- 既存text企業IDと企業UUIDのbridge方式は正式設計に対する別承認後まで未決定とし、Phase 1Aのschemaへ移行専用列を追加しない。
...
- 新規Auth tableは各table-creation migrationの同一transaction内でRLSをenableし、後続policy migrationまでuser allow policyなしのdeny-by-defaultとする。
...
- 必須table、PK、FK、unique、CHECK、active ownerを最大1件に制限する部分一意index、RLS enabled、user allow policy不在が満たされないため失敗するpgTAP。
...
2. 各tableを作成するmigrationの同一transaction内でRLSをenableし、利用者allow policyは追加しない。
3. 正式設計にない移行専用company-key列は追加せず、UUID bridge方式を別承認後へdeferする。
4. timestampは`timezone`付きとし、table別timestamp/FK action matrixの承認内容に従う。
5. local DBへresetしてpgTAPをGREENにする。
6. 既存Backend 62件が無変更でGREENであることを確認する。

**ロールバック:** 利用開始前なら新規テーブルを依存順にdropするdown手順をrunbookへ記録する。本番適用後はmigration fileを編集せず新しいforward migrationを使う。

**最小実装フェーズ:** preflight gate解除後に最初に実装する候補はこのPhase 1Aとする。既存認証・業務経路に触れず、正式設計のデータ制約をlocal DBで検証できる最小単位だからである。

### Phase 1B: private helperと企業UUID橋渡し

**依存:** Phase 1A、承認済みcompany mapping、正式設計に対して別承認されたUUID bridge方式

**変更・作成ファイル:**

- Create: `supabase/migrations/202607230003_auth_private_functions.sql`
- Create: `supabase/migrations/202607230004_auth_table_policies.sql`
- Create: `supabase/migrations/202607230005_business_company_uuid_bridge.sql`
- Create: `supabase/tests/002_auth_helpers.test.sql`
- Create: `supabase/tests/003_auth_table_policies.test.sql`
...
2. Phase 1AでRLS enabled済みの新規認証テーブルへ、承認済みのuser allow policyと最小grantを追加する。profilesは本人最小情報、membershipは本人所属、招待は権限経路に限定する。
...
- hybrid routingのRED testとして、(a) Basicだけでブラウザ入口へ来たsessionless requestはBasic検証後にlegacy branchが選ばれ、Basic credentialを転送せずサーバー側の既存`X-Admin-Key`で転送できない、(b) valid Supabase BearerだけのrequestはSupabase branchで成功できない、(c) invalid/expired Bearerにvalid `X-Admin-Key`を添えてlegacy成功へfallbackできる、(d) Bearerと`X-Admin-Key`の両credential classを持つrequestを拒否できない、(e) Basic、Bearer、`X-Admin-Key`のいずれもないrequestを拒否できないテスト。
- Next.js proxyが未検証のcookie/sessionをSupabase branchとして転送する、Bearerや選択企業をブラウザ入力から転送する、Basic credentialまたはsecretをbackendやレスポンスへ出すテスト。
- callbackの`next`が外部URLへopen redirectできるテスト。
- password resetまたは認証済みpassword change後にSupabase sessionをglobal sign-outせず、そのuserの全`app_sessions`を失効せず、変更前のaccess/refresh tokenまたはapp sessionでAPIを継続利用できるテスト。
- password reset/change完了後に旧sessionのまま継続できる、または新規loginとTOTPによる`aal2`再到達なしに保護routeへ到達できるテスト。
...
6. `frontend/middleware.ts` に `AUTH_MODE=legacy|hybrid|supabase` を導入する。hybridではcredential classを排他的に選ぶ。検証対象の`Authorization: Bearer`が存在するrequestはSupabase branchだけで検証し、JWT、expiry、AAL、app session、membershipのいずれの失敗後も`X-Admin-Key`へfallbackしない。Bearerと`X-Admin-Key`の両方が存在するrequestはcredential confusionとして400/401で明示的に拒否する。
7. Route Handlerは検証済みaccess tokenを `Authorization: Bearer` で転送し、選択企業を署名済みサーバーセッションから `X-Company-ID` として転送する。ブラウザ入力だけを信用しない。
8. sessionless legacy requestが既存`X-Admin-Key`経路を使えるのは`AUTH_MODE=hybrid`で、Next.jsのBasic保護入口がBasicを検証してlegacy branchを選んだ場合だけとする。Basic credentialはブラウザ入口専用で、identity header、Bearer、backend credentialとして転送しない。Route Handlerはこのlegacy branchでだけサーバー側の既存`X-Admin-Key`を付与し、Supabase branchには付与しない。FastAPIは`Authorization`、`X-Admin-Key`の排他的な組合せを同じ規則でfail closedに判定する。
9. login、callback、forgot/reset、認証済みpassword changeの最小画面とserver action/routeを追加する。公開ルートallowlistを狭く保つ。
10. password resetまたはpassword changeが成功したら、本人のSupabase Auth sessionを管理APIでglobal sign-outし、そのuserの現在を含む全`app_sessions`へ`revoked_at`を設定する。どちらか一方だけの失効を成功扱いにせず、旧access/refresh tokenと旧app sessionをfail closedで拒否する。完了後は新規loginを要求し、TOTPを再検証して新しい`aal2` sessionと`app_sessions`行を発行するまで保護routeを許可しない。
11. このフェーズでは全業務routeを一括置換せず、read-only health/auth確認routeから導入する。
12. 正式設計のHTTP契約を共通error modelで固定する: 無効JWTは401、MFA未完了は403 `MFA_REQUIRED`、30日超過は401 `SESSION_EXPIRED`、停止membershipは403 `MEMBERSHIP_SUSPENDED`、read-only企業へのwriteは403 `COMPANY_READ_ONLY`、停止企業は403 `COMPANY_SUSPENDED`、role不足は403 `INSUFFICIENT_ROLE`、期限切れ招待は410 `INVITATION_EXPIRED`。業務データIDの他社所属は404を優先し、存在を漏らさない。
...
1. 次の正式audit-event matrixを必須実装範囲とし、各イベントのsuccess/failure、actor、target、company、結果、timestamp、request correlationを同じschemaで監査する。表のイベント群を「任意」または後続対応へ落とさない。

| イベント群 | 必須audit event |
|---|---|
| Auth/session/password | login success/failure、logout、session revoke/expire、password reset/change、reset/change後のglobal sign-out |
| 招待 | create、resend、accept、expire、revoke |
| role/owner移譲 | role grant/change/revoke、owner transfer開始/完了/失敗 |
| 企業 | create、metadata change、status change、company switch success/failure |
| MFA | enroll、challenge/verify success/failure、reset |
| export | request、generate success/failure、download、expire/delete |
| 設定 | FAQ、app、question tree、applicant-status設定のcreate/update/delete |
| 応募者状態 | applicant status change |
| LINE/message | LINE send success/failure、message action、Webhook accept/reject/process failure |
| 面接 | interview create/update/reschedule/cancel |
