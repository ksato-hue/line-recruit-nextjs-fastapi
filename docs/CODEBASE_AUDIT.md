...
### 2026-07-24 legacy admin HTML removal follow-up

**Fact:** Repository-wide searches found no current Next.js caller, production-code redirect, health check, or external-form link for GET `/applicants-view`, GET `/applicant/{applicant_id}`, or GET `/inquiries-view`. The only production links were generated inside those three HTML responses. The Next.js client uses the server-side `/api/admin/*` proxy and the FastAPI `/api/applicants*` and `/api/inquiries*` JSON APIs instead (`frontend/lib/api.ts`; `frontend/app/api/admin/[...path]/route.ts`).

**Fact:** The removed routes interpolated DB-derived applicant names, phone numbers, jobs, motivations, statuses, interview values, memos, LINE user IDs, and inquiry messages directly into f-string HTML without an escaping layer. This exposed a stored-XSS path whenever attacker-controlled stored text reached the page.

**Decision applied:** Remove the three redundant HTML routes and their route-local HTML/CSS/navigation code. Remove `HTMLResponse` because no remaining route uses it. Preserve the company-scoped compatibility JSON GET `/applicants`, modern `/api/applicants*` and `/api/inquiries*` routes, `/api/health`, `/webhook`, shared Supabase access, and admin-key dependencies.

**TDD evidence:** Before deletion, the seven-test removal suite had exactly three failures: each target URL returned 200 instead of the required 404. JSON applicant/inquiry APIs, health, and Webhook registration already passed. After deletion, the regression suite verifies the three 404 responses with a valid admin key, JSON response availability and shape, the missing-key 401 boundary, health, and Webhook registration (`backend/tests/test_legacy_routes_tenant_scope.py`).
