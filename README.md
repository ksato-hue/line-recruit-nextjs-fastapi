...
4. [`202607210001_applicant_tags.sql`](supabase/migrations/202607210001_applicant_tags.sql)
...
- 定期送信ジョブは未接続です。リマインドの名前、有効状態、送信までの時間・単位、本文は可変リストとして保存できますが、自動送信は行われません。旧1時間・24時間・3日設定は初回表示時にリストへ変換され、旧設定値も削除しません。
